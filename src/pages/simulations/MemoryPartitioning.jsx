...
function allocate(partitions, processes, strategy) {
  // Create a deep copy of partitions to track usage
  ...
  for (const proc of processes) {
    let chosen = null
    // Filter partitions that haven't been used and are large enough
...
export default function MemoryPartitioning() {
  ...
  // Lab Analysis: Check for External Fragmentation
  ...
  return (
    <div className="min-h-screen pt-28 pb-20 px-4 max-w-6xl mx-auto bg-[#0a0a0f]">
      <Link to="/" className="inline-flex items-center gap-2 text-gray-400 hover:text-fuchsia-400 mb-8 transition-colors">
        <ArrowLeft size={16} /> Back to Dashboard
      </Link>

      <div className="mb-8">
        <h1 className="text-4xl font-extrabold text-white mb-2">Memory Partitioning <span className="text-fuchsia-500">Lab</span></h1>
        <p className="text-gray-400">Fixed partition allocation simulation with fragmentation analysis.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
        ...
        <div className="lg:col-span-3 glass-card p-6 border-white/10">
          ...
          <div className="flex flex-wrap gap-3">
            {['First Fit', 'Best Fit', 'Worst Fit'].map(s => (
              <button 
                ...
                className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${strategy === s ? 'bg-fuchsia-500 text-white shadow-lg shadow-fuchsia-500/20' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}
              >
                {s}
              </button>
            ))}
        ...
        <div className="glass-card p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-white font-bold text-sm uppercase tracking-wider">Partitions (KB)</h3>
          ...
          <div className="space-y-3">
            {partitions.map((size, i) => (
              <div key={i} className="flex items-center gap-2 group">
                <div className="w-1.5 h-8 rounded-full" style={{ background: COLORS[i % COLORS.length] }} />
                <input 
                  type="number" 
                  value={size} 
        ...
        <div className="lg:col-span-2 glass-card p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-white font-bold text-sm uppercase tracking-wider">Incoming Processes (KB)</h3>
            <button onClick={addProcess} className="text-fuchsia-400 hover:text-white transition-colors"><Plus size={18} /></button>
          </div>
            ...
            {processes.map((p, i) => (
              <div key={i} className="flex items-center gap-2 bg-white/5 p-2 rounded-xl border border-white/5">
                <input 
                  ...
                  className="w-16 bg-transparent text-fuchsia-400 font-bold outline-none px-2"
                />
                <input 
                  type="number" 
                  value={p.size} 
                  onChange={e => updateProcess(i, 'size', e.target.value)}
                  className="flex-1 bg-black/20 border border-white/10 rounded-lg px-3 py-1 text-white text-sm outline-none"
                />
                <button onClick={() => removeProcess(i)} className="p-2 text-gray-600 hover:text-red-500"><Trash2 size={14} /></button>
              </div>
            ))}
          </div>
          <div className="mt-8 flex gap-4">
            <button onClick={calculate} className="flex-1 bg-fuchsia-500 hover:bg-fuchsia-400 text-white font-bold py-3 rounded-2xl flex items-center justify-center gap-2 transition-all">
              <Play size={18} fill="currentColor" /> Run Allocation
            </button>
            <button onClick={reset} className="px-6 border border-white/10 text-gray-400 hover:text-white rounded-2xl transition-all">
              <RotateCcw size={18} />
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="lg:col-span-1">
            <div className="glass-card p-6 sticky top-24">
              <h3 className="text-white font-bold mb-6 text-sm uppercase tracking-widest">Memory Stack View</h3>
                ...
                {result.parts.map((part, i) => {
                  ...
                  return (
                    <div 
                      ...
                      style={{ height: `${(part.size / totalPartitionsSize) * 600}px`, minHeight: '80px' }}
                    >
                      {/* Process (Dark/Filled) */}
                      {part.used > 0 && (
                        <div 
                          className="w-full flex flex-col items-center justify-center text-[11px] font-black text-black transition-all"
                      ...
                      {/* Fragmentation (Light/Striped) */}
                      {part.free > 0 && (
                        <div 
                          className="w-full flex items-center justify-center text-[10px] font-bold transition-all relative"
                          style={{ 
                            height: `${freePct}%`, 
                      ...
                      {/* Labels */}
                      <div className="absolute right-3 bottom-1 text-[9px] text-gray-600 font-mono">
                        Partition {i+1}: {part.size}KB
                      </div>
                    </div>
                  )
                })}
              ...
              {/* Legend */}
              <div className="mt-8 space-y-3 bg-black/20 p-4 rounded-xl border border-white/5">
                <div className="flex items-center gap-3 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                  <div className="w-4 h-4 rounded bg-fuchsia-500" /> Allocated Process
                </div>
                <div className="flex items-center gap-3 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                  <div className="w-4 h-4 rounded opacity-30" style={{ backgroundImage: `repeating-linear-gradient(45deg, #fff, #fff 2px, transparent 2px, transparent 4px)` }} /> Internal Fragmentation
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {externalFragProcesses.length > 0 && (
              <div className="bg-amber-500/10 border border-amber-500/20 p-6 rounded-3xl flex gap-4">
                ...
                <div>
                  <h4 className="text-amber-500 font-bold mb-1">External Fragmentation Analysis</h4>
                  <p className="text-gray-400 text-sm leading-relaxed">
                    Processes <b>{externalFragProcesses.map(p => p.process).join(', ')}</b> failed to allocate. 
                    The system has {result.parts.reduce((a,p)=>a+p.free,0)}KB of free memory, but it is split into non-contiguous blocks that are each too small for these requests.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="glass-card overflow-hidden">
              <div className="p-6 border-b border-white/5">
                <h3 className="text-white font-bold">Allocation Table — {strategy}</h3>
              </div>
                ...
                <tbody className="divide-y divide-white/5">
                  {result.results.map((r, i) => (
                    <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="p-4 text-gray-400">{r.size} KB</td>
                      <td className="p-4 text-fuchsia-400 font-mono">{r.partition ? `PART-${r.partition}` : 'NONE'}</td>
                      <td className="p-4 text-orange-400 font-mono">{r.partition ? `${r.internal} KB` : '—'}</td>
                      <td className="p-4">
                        <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase ${r.partition ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
                          {r.status}
                        </span>
                      </td>
                    </tr>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="glass-card p-6 border-orange-500/20">
                <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-1">Total Internal Fragmentation</div>
                <div className="text-3xl font-black text-orange-400">
                  {result.results.reduce((s, r) => s + (r.partition ? r.internal : 0), 0)} <span className="text-sm font-normal">KB</span>
                </div>
              </div>
              <div className="glass-card p-6 border-fuchsia-500/20">
                <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-1">Physical Free Memory</div>
                <div className="text-3xl font-black text-fuchsia-400">
                  {result.parts.reduce((acc, curr) => acc + curr.free, 0)} <span className="text-sm font-normal">KB</span>
                </div>
              </div>
            </div>
          </div>
